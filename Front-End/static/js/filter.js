...
function createDropDown(data){
    ...
    dataM = data.filter(d =>d.FSA.substring(0,1)=="M");
      //update!!
    FSAList =  dataM.map(d => d.FSA).reverse();
    FSAList = FSAList.filter((item, i, ar) => ar.indexOf(item) === i).sort();
    var dropDownFSA= d3.select("#fsa_filter")
    dropDownFSA.append("option").text("All FSA");
    //give the dropdow id values
      //update!!
    FSAList.forEach(element => {
  ...
  //update!!
function getFilterParm(PriceMin, PriceMax, BedroomsMin, BedroomsMax, FilteredFSA){
