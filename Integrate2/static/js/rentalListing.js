...
//http://127.0.0.1:5000/recommend?id=c_7241928903&price=[0,-1]&bedrooms=[0,-1]&FSA=M6J
    ...
    d3.json((baseUrl+recommendURL), function(data){
        ...
        if(data.length == 3){
            ...
            d3.selectAll("#ThreerecommendationTitle").append("h3").text('Recommendations based on your Selection');

            //add imgs to One
            if(element.target.options.icon.options.image != null && ((typeof data[0].image) == "string") && (data[0].image.length > 10)){
                d3.select("#ThreerecommendationOneImg").attr("src", data[0].image).style("width", "100%");
            }else{
                d3.select("#ThreerecommendationOneImg").attr("src", coming_soon).style("width", "100%");
            }

            //add imgs to Two
            if(data[1].image != null && ((typeof data[1].image) == "string") && (data[0].image.length > 10)){
                d3.select("#ThreerecommendationTwoImg").attr("src", data[1].image).style("width", "100%");
            }else{
                d3.select("#ThreerecommendationTwoImg").attr("src", coming_soon).style("width", "100%");
            }

            //add imgs to Three
            if(data[2].image != null && ((typeof data[2].image) == "string") && (data[2].image.length > 10)){
                d3.select("#ThreerecommendationThreeImg").attr("src", data[2].image).style("width", "100%");
            }else{
                d3.select("#ThreerecommendationThreeImg").attr("src", coming_soon).style("width", "100%");
            }

            //add info to first recommendation
            d3.selectAll("#ThreerecommendationOneInfo").append("h3").text(data[0].title).classed('card-title', true).style("width", "100%");
            d3.selectAll("#ThreerecommendationOneInfo").append("p").text(" ").classed('card-text', true).style("width", "100%");
            ...
            d3.selectAll("#ThreerecommendationOneInfo"").select("p").html(`Source: <a target="_blank" href="${element.target.options.icon.options.url}">${element.target.options.icon.options.source}</a>`).classed('card-text', true);

            //add info to second recommendation
            d3.selectAll("#ThreerecommendationTwoInfo").append("h3").text(data[1].title).classed('card-title', true).style("width", "100%");
            d3.selectAll("#ThreerecommendationTwoInfo").append("p").text(" ").classed('card-text', true).style("width", "100%");
            ...
            d3.selectAll("#ThreerecommendationTwoInfo"").select("p").html(`Source: <a target="_blank" href="${element.target.options.icon.options.url}">${element.target.options.icon.options.source}</a>`).classed('card-text', true);
            //add info to Third recommendation
            d3.selectAll("#ThreerecommendationThreeInfo").append("h3").text(data[2].title).classed('card-title', true).style("width", "100%");
            d3.selectAll("#ThreerecommendationThreeInfo").append("p").text(" ").classed('card-text', true).style("width", "100%");
            ...
            // d3.selectAll("#ThreerecommendationThreeInfo"").select("p").html(`Source: <a target="_blank" href="${element.target.options.icon.options.url}">${element.target.options.icon.options.source}</a>`).classed('card-text', true);
        }
